...
void
BitmapUtilities::RasterToPolygonsMoore(BBitmap* bitmap, BRect bounds, BList* polygons)
{
		...
		for (int32 x = leftb; x <= rightb; x++) {
			...
			if (included_points->HasPoint(x, y) && inside == false)
				...
				while (true) {
					...
					bool containsPoint
						= (GetPixel(bitmap, (int32)check.x, (int32)check.y) != 0x00);
					if (containsPoint) {
						...
						if (check == startPos) {
							...
							if (newCheckLocation == 1 || end_counter >= 3) {
								inside = true;
								break;
							}
						}
				...
				if (point_count > 1) {
					...
					if (border == true)
						...
						new_polygon
							= new HSPolygon(point_list, point_count, HS_POLYGON_COUNTERCLOCKWISE);
					}
